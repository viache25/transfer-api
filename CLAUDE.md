...
- `config/` — `SecurityConfig` (Spring Security filter chain), `ApiKeyAuthenticationFilter` (reads `X-API-Key`, looks up `Terminal` by SHA-256 hash), `ApiKeyHasher`, `RestAuthenticationEntryPoint` (401 as `ProblemDetail`, since Spring Security's filter runs before `GlobalExceptionHandler` gets a chance), and `DevTerminalSeeder` (`@Profile("dev")` `CommandLineRunner` that seeds one terminal — see Authentication below).
